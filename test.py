...
class RandomWithTaskAmongUsEnv(gym.Env):
    ...
    def __init__(self, grid_shape=(11, 11), n_imposters=1, n_crew=3, imposter_view=2,
                 full_observable=True, kill_reward=5, task_reward=0.2, missed_kill_reward=0.5):
        ...
        self.action_space = MultiAgentActionSpace([spaces.Discrete(4) for _ in range(self.n_imposters)])
    ...
    def legal_actions(self, simulation=True):
        actions = []
        if self.imposter_pos[0][1] > 0:
            actions.append(1)
        if self.imposter_pos[0][1] < 10:
            actions.append(3)
        if self.imposter_pos[0][0] > 0:
            actions.append(2)
        if self.imposter_pos[0][0] < 10:
            actions.append(0)
        return actions
   
    def sample_action(self, simulation=True):
    ...
    def get_copy(self):
        ...
        env.imposter_pos = copy.deepcopy(self.imposter_pos)
        env.crew_pos   = copy.deepcopy(self.crew_pos)
        env.crew_alive            = copy.deepcopy(self.crew_alive)
        env._base_grid            = copy.deepcopy(self._base_grid)
        env._full_obs             = copy.deepcopy(self._full_obs)
        env.all_dones             = copy.deepcopy(self.all_dones)
        env.observation_space     = copy.deepcopy(self.observation_space)
        env.task_completion       = copy.deepcopy(self.task_completion)
        env._step_count           = copy.deepcopy(self._step_count)
        env._steps_beyond_done    = copy.deepcopy(self._steps_beyond_done)
        env._imposter_dones       = copy.deepcopy(self._imposter_dones)
        env._crew_alive           = copy.deepcopy(self._crew_alive)
        env._total_episode_reward = copy.deepcopy(self._total_episode_reward)
    ...
    def __update_imposter_pos(self, imposter_i, move):
        ...
        if type(move) is list: move = move[0]
        next_pos = None
        if move == 0:  # down
    ...
    def step(self, imposters_action, simulation=True):
        ...
        if type(imposters_action) is not list: imposters_action = [imposters_action]
        ...
        for crew_i in range(self.n_crew):
            if self._crew_alive[crew_i]:
                ...
                if abs(self.imposter_pos[0][0] - pos[0]) <= 1 and abs(self.imposter_pos[0][1] - pos[1]) <= 1:
        ...
        return init_state, reward, end_state, self._imposter_dones[0]
    ...
    def __kill_neighbour_coordinates(self, pos):
        neighbours = []
        min_x = pos[0]-1
        max_x = pos[0]+1
        min_y = pos[1]-1
    ...
    def render(self, mode='human'):
        ...
        for imposter_i in range(self.n_imposters):
            for neighbour in self.__kill_neighbour_coordinates(self.imposter_pos[imposter_i]):
                fill_cell(img, neighbour, cell_size=CELL_SIZE, fill=PREY_COLOR, margin=0.1)
            fill_cell(img, self.imposter_pos[imposter_i], cell_size=CELL_SIZE, fill=PREY_COLOR, margin=0.1)
...
def run_game(output_path):
    ...
    with open('./easier/rewards.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
init_time = time.time()
n_threads = 500
output_files = [f'./easier/game_{i}.csv' for i in range(n_threads)]

with open(f'./easier/rewards.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['r'])
